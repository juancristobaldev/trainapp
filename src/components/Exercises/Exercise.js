...
const Exercise = ({ children, item, deleteExerciseOfList }) => {
  ...
  return (
    <Container className={`exercise ${darkMode && "darkMode"}`} key={item.name}>
      <Container
      ...
      >
        <Text className={"exercise-name"} text={item.name} />
        <Container className={"exercise-info"}>
          <Text className={"exercise-muscle"} text={item.muscleEx} />
          <Text className={"exercise-type"} text={item.typeEx} />
        </Container>
