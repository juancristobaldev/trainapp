...
import {
  GET_EXERCISES_BY_TOKEN,
  GET_ROUTINES_AND_USER_BY_TOKEN,
} from "../../data/query";
...
const RoutineCrud = ({ routineObj }) => {
  ...
  const { listForSelect, updateListForSelect, deleteItem } = useList(
    "exercises",
  ...
  const { loadingStatus, deleteSomeExercise } = useExercises(
    token,
    { list: listForSelect, updateList: updateListForSelect },
    { stateValue: state, setState: setState }
  );

  const { checkSerie, addSerie, deleteSeries, classControl } = useSeries({
    state: state,
    updateState: setState,
  });

  const [createRoutine, { loading: loadingCreateRoutine }] =
      useMutation(CREATE_ROUTINE),
    [updateRoutine, { loading: loadingUpdateRoutine }] =
    ...
    if (name !== "name") {
      ...
      await newList.forEach((item) => {
        ...
        if (item.idList === idList) {
          const indexSerie = item.seriesEx.findIndex(
            (item) => item.idSerie === serie
          );
  ...
  const handleSubmit = async (e, confirmation) => {
    ...
    if ((active && id) || (!active && id)) {
      ...
      if (error)
        await setState({
          ...state,
          dataFormCreate: dataRoutine,
          modalUncompletedRoutine: true,
        });
      else {
        ...
        if (active && id) {
          ...
          const timeNow = document
            .getElementById("progressive-count")
            .innerHTML.substring(18, 26);
          ...
          if (dataRoutine.timeRecord !== "indefinido") {
            ...
            const objectRecord = {
              hourRecord: parseInt(
                `${dataRoutine.timeRecord[0]}${dataRoutine.timeRecord[1]}`
              ),
              minRecord: parseInt(
                `${dataRoutine.timeRecord[3]}${dataRoutine.timeRecord[4]}`
              ),
              segRecord: parseInt(
                `${dataRoutine.timeRecord[6]}${dataRoutine.timeRecord[7]}`
              ),
            };
            ...
            const { hourRecord, minRecord, segRecord } = objectRecord,
              ...
              timeRecordString = `${
                ...
                timeRoutine.min <= 9 ? `0${timeRoutine.min}` : timeRoutine.min
              }:${
                timeRoutine.seg <= 9 ? `0${timeRoutine.seg}` : timeRoutine.seg
              }`;
            ...
            if (hourRecord == hour) {
              if (minRecord == min) {
                if (segRecord == seg) dataRoutine.timeRecord = timeRecord;
                  ...
                  if (segRecord > seg)
                    dataRoutine.timeRecord = timeRecordString;
      ...
      if (!exercises.length)
        errorObj.exercises = "Debes agregar al menos un ejercicio.";

      if (!name.length)
        errorObj.name = "Debes escribir un nombre para tu rutina.";
      ...
      if (!errorArray.length) {
        const inputVariables = {
          ...dataFormCreate,
          exercises: JSON.stringify(dataFormCreate.exercises),
        };
        ...
        setState({
          ...state,
          modalErrors: { error: true, errors: errorArray },
          errors: errorObj,
        });
    }
  };
  useEffect(() => {
    if (routine) {
      ...
      newListOnCreate.forEach((item) => {
        item.seriesEx.forEach((serie) => {
          const lastSerie = serie.other
            ? `${serie.other} Kg / ${serie.reps} Reps`
            : serie.time
            ? `${serie.time} Min`
            : `${serie.reps} Reps`;
  ...
  return (
    <>
      ...
      <Section
        className={`grid ${widthScreen > 650 && "web"} ${
          darkMode && "darkMode"
        }`}
      >
        <Section
          className={`section-create-routine ${widthScreen > 650 && "web"}`}
        >
                  ...
                  state.timer.time === false
                    ? () =>
                        setState({
                          ...
                          timer: {
                            ...state.timer,
                            modalTimer: !state.timer.modalTimer,
                            time: "",
                          },
                        })
                    : () =>
                        setState({
                          ...
                          timer: {
                            ...state.timer,
                            secondPlane: !state.timer.secondPlane,
                          },
                        })
                }
          ...
          >
            {active && id ? (
              <Container className={"stats-goroutine"}>
                <h2>{routine.name}</h2>
                <Text text={`Mejor tiempo 🎉: ${routine.timeRecord}`} />
                <ProgressiveCount id={"progressive-count"} />
              </Container>
            ) : (
              <FormControl
            ...
            <List
              ...
              render={(exercise) => (
                ...
                >
                  ...
                  >
                    {widthScreen < 650 && (
                      <Container className={"container-add-serie"}>
                        <Button
        ...
        <Modal>
          ...
          {state.modalDelete.boolean && (
            <Modal>
              ...
              <Container
                className={`back ${darkMode && "darkMode"}`}
                onClick={() =>
                  setState({ ...state, modalDelete: { boolean: false } })
                }
              />
            </Modal>
          )}
          {state.modalUncompletedRoutine === true && (
            <ModalAreUSure
              ...
              cancelFunction={() =>
                setState({ ...state, modalUncompletedRoutine: false })
              }
            />
          )}
        </Modal>
