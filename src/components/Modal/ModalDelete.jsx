...
const ModalDelete = ({ token, objectState, exercise }) => {
  ...
  const { listForSelect, updateListForSelect } = useList(
    "exercises",
  ...
  return (
    <Container className={`modal-delete ${exercise && "exercise"}`}>
      <Container className={"container-text"}>
        <Text
          text={"¿Estas seguro que deseas eliminar los siguientes ejercicios?"}
        />
      </Container>
      {exercise && (
        <Container className={"text-warning"}>
          <Text
            text={
              "Al eliminar estos ejercicios no se eliminiran de tus rutinas creadas, pero ya no podras seleccionarlo para rutinas futuras."
            }
          />
        </Container>
      )}
      <Container className={"list-exercises-delete"}>
        {state.modalDelete.items.map((item) => (
          <Text key={item.name} text={item.name} style={{ color: "red" }} />
        ))}
      </Container>
