...
const TimerMenu = ({ objState }) => {
  ...
  const setTimer = async () => {
    const clock = state.timer.clock,
      errors = [];
    let time = "";
    if (parseInt(clock.minutes) > 59 || parseInt(clock.seconds) > 59)
      errors.push("El tiempo maximo es: 59:59min");
    if (!clock.minutes || !clock.seconds)
      errors.push("Minutos y segundos obligatorios");
    if (errors.length === 0) {
      const newList = [...JSON.parse(localStorage.getItem("timer"))];
      time = `${
        clock.minutes <= 9 ? `0${clock.minutes}` : `${clock.minutes}`
      }:${
        clock.seconds <= 9 && clock.seconds >= 1
          ? `0${clock.seconds}`
          : `${clock.seconds}`
      }`;
      if (newList.length < 3) {
      ...
      setState({
        ...state,
        timer: { ...state.timer, clock: {}, type: "select" },
      });
    } else {
      setState({
        ...state,
        timer: { ...state.timer, errors: { error: true, errors: [...errors] } },
      });
    }
  };
  ...
  return (
    <>
      <Container
        ...
        style={
          state.timer.secondPlane ? { display: "none" } : { display: "block" }
        }
      />
      {!state.timer.time ? (
        <>
          {state.timer.type === "select" ? (
            <Container
              className={`modal-timer ${
                state.timer.secondPlane ? "second-plane" : "undefined"
              } ${darkMode && "darkMode"}`}
            >
              ...
              <List
                ...
                item={timer}
                onEmpty={() => (
                  <Text text={"No has creado ningun temporizador"} />
                )}
                render={(timer) => (
              ...
              />
              <Container className={"buttons-timer-select"}>
                <Button
                  textButton={"Crear temporizador"}
                  onClick={() =>
                    setState({
                      ...state,
                      timer: { ...state.timer, type: "create" },
                    })
                  }
                />
              </Container>
              ...
              <Container className={"close-button"}>
                <MdOutlineKeyboardReturn
                  ...
                  onClick={() =>
                    setState({
                      ...state,
                      timer: { ...state.timer, type: "select", time: "" },
                    })
                  }
                />
              </Container>
